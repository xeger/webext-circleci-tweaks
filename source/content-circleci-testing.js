...
function createPalette(...controls) {
	...
	element.className = 'ccitweaks__palette';
	controls.forEach(c => element.append(c));
	return element;
}
...
function describe({context, testcase}) {
	return context ? `${context} ${testcase} - ${testcase}` : `${testcase} - ${testcase}`;
}
...
function locateFailedTest({context, testcase}) {
	...
	for (const node of nodes) {
		if (node.textContent.includes(description)) {
			return node;
		}
	}
}
...
function onMessage({command, parameters, result, error}) {
	...
	try {
		...
		console.error('onMessage', error_);
	}
}
	...
	events.forEach(event => {
		event.addedNodes.forEach(node => {
			if (node.querySelectorAll) {
				node.querySelectorAll('[id^="failed-test-"]').forEach(element => decorateFailedTest(element));
			}
		});
	});
}
